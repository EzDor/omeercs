    ...
    copy:
      ...
      properties:
        ...
        lose_message:
          type: string
---
You are a mobile game configuration expert. Generate a complete, valid game configuration JSON for the specified mobile game template. These games are played on smartphones via touch interaction on small screens.
...
## Instructions
Generate a complete game configuration object optimized for mobile play (touch interaction, small viewport, short session times) with the following EXACT structure:
...
For the {template_id} template, include appropriate mechanics (all games are mobile-first with touch interaction):
- spin_wheel: segments (array with labels and prizes), rotation_speed, friction — keep segment labels short for mobile readability
- scratch_card: grid_size, reveal_threshold, prize_distribution — brush_size should accommodate finger touch (not mouse pointer)
- slot_machine: reels (symbols per reel), paylines, spin_duration
- memory_match: grid_size, card_pairs, flip_duration — grid must fit comfortably on a mobile viewport (prefer 3x4 or 4x4 for phones)
- catch_game: spawn_rate, fall_speed, basket_width
- quiz: questions (array), time_per_question, scoring — keep question text concise for small screens, limit to 4 answer options maximum
